...
CHILD_MARKER = "HOSTS_MAKE_SPACE_CHILD"
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="hosts-make-space-") as temporary:
        root = Path(temporary)
        ...
        subprocess.run(
            [environment.get("HOSTS_MAKE", "make"), "-f", str(copied / "Makefile"), "check"],
            cwd=caller,
            env=environment,
            check=True,
            timeout=180,
